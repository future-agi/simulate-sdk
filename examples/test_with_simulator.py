...
async def run_test():
    print("\nStarting test with Future AGI Simulate SDK (simulator config)...")
    ...
    scenario = Scenario(
        ...
        dataset=[
            Persona(
                persona={"name": "Bob", "age": 32, "mood": "frustrated"},
                situation="He is having trouble updating his payment information",
                outcome="Successfully update payment details and confirm next billing date",
            ),
        ],
    )

    # Use a lightweight simulator for the customer
    ...
    runner = TestRunner()
    report = await runner.run_test(agent_definition, scenario, simulator)
