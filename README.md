...
metrics, WebRTC RTP/track/codec/audio-level stats with jitter and packet-loss
counters, audio artifacts, timelines, and voice trace artifacts. `load_pipecat_frame_log` turns exported
...
checkpoint/session, subgraph/node, final-output, and state evidence from stream
exports.
...
report = await TestRunner().run_test(
    ...
    environment=[
        ...
        FrameworkLifecycleEnvironment(
            trace=normalize_framework_lifecycle_trace(
                framework="langgraph",
                session_id="thread-123",
                phases=[
                    {"stage": "initialize", "status": "completed"},
                    {"stage": "register_tools", "tools": ["search_order"]},
                    {"stage": "checkpoint", "state_persistence": True},
                    {"stage": "shutdown", "cleanup": True},
                ],
            ),
        ),
        OrchestrationTraceEnvironment(
...
# Optimizer society trace replay:
environment = OptimizerTraceEnvironment(trace=optimizer_society_trace)
...
See [`examples/local_voice_webrtc_stats_replay.py`](examples/local_voice_webrtc_stats_replay.py) for a LiveKit/WebRTC `getStats()` replay cookbook with RTP, track, codec, audio-level, jitter, and packet-loss checks.
...
See [`examples/local_agent_control_plane.py`](examples/local_agent_control_plane.py) for a framework-neutral runtime control-plane cookbook with action gates, approvals, rollback, kill switch, circuit breakers, rate limits, budgets, audit, containment, drift detection, and incident scoring.
...
See [`examples/local_multi_agent_orchestration_trace.py`](examples/local_multi_agent_orchestration_trace.py) for a multi-agent orchestration control cookbook with spawn, delegation, communication, aggregation, stop, delegate-route, terminal status, and state checks.
...
coverage, framework trace coverage, framework runtime coverage/contract, framework lifecycle coverage/quality, framework transcript quality, world contract coverage/quality,
...
multi-agent trace coverage, multi-agent coordination quality, optimizer trace
coverage, optimizer trace quality, artifact coverage,
...
evaluation = evaluate_agent_report(
    ...
    config={
        ...
        "required_voice_trace": ["audio", "vad", "stt", "tts", "interruption", "route", "frame", "noise", "overlap", "timeline", "media", "sample_rate", "duration", "rms", "peak", "webrtc", "rtp", "track", "codec", "audio_level", "timing_distribution", "timing_stage", "eou", "llm", "turn"],
...
See [`examples/local_domain_package_registry.py`](examples/local_domain_package_registry.py) for a versioned registry that overrides package aliases, required fields, statuses, required documents, and tolerances.
...
- [x] Local `evaluate_agent_report` scoring for trajectory, trajectory templates, agent goal accuracy, tool-call accuracy/F1, policy adherence, tools, memory correctness, multimodal faithfulness, observability replay coverage/quality, agent trust-boundary and control-plane coverage/quality, optimizer trace coverage/quality, framework trace coverage, framework runtime coverage/contract, framework lifecycle coverage/quality, framework transcript quality, orchestration trace coverage/flow quality, streaming trace coverage/interaction quality, world contract coverage/quality, adversarial resilience, cross-trial memory/skill quality, retrieval/memory attribution, source contradiction, artifact grounding quality, artifact semantics quality, domain package quality, autonomy-loop coverage, autonomy-loop quality, multi-agent trace coverage, multi-agent coordination quality, browser/CUA action outcome and grounding quality, browser trace coverage, browser mutation resilience, voice trace coverage, voice interaction quality, environment injection, and pentest signals
...
- [ ] Streaming WebRTC media decoding beyond stats replay, streaming diarization, and external perceptual-model integrations
